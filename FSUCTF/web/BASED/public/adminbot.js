...
async function checkout(base_url, admin_password, report_id) {
    ...
    try {
        ...
        console.log(`Failed to navigate to ${url}! Error: ${error.message}`);
        await browser.close(); // Ensuring browser closes even on error.
        process.exit(1);
    }
    ...
    try {
        ...
        console.log(`Failed to close browser! Error: ${error.message}`);
        process.exit(1);
    }
}
